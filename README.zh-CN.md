...
- **极小体积** —— SDCC 版仅占 968 字节 Flash（共 8 KB）、150 字节 XRAM；
  Keil 版约 1.9 KB（多链接了一个浮点 HSV→RGB 函数，留作彩虹/渐变扩展钩子）。
- **附预编译 hex**（[prebuilt/](prebuilt/)）—— 不装工具链也能直接烧录。
...
└── prebuilt/               ← 可直接烧录的 hex（必须配 35 MHz IRC）
```
...
**不装工具链**：直接烧 [prebuilt/](prebuilt/) 里的 hex
（推荐 `ws2812_stc8g1k08a_sdcc_35mhz.hex`，数据脚为推挽输出）。

## 烧录

STC 单片机通过出厂串口引导程序烧录，只需一个 **USB 转 TTL 串口模块**
（CH340 类，几块钱）。

1. 接线：模块 TXD → P3.0(RxD)，模块 RXD → P3.1(TxD)，GND → GND，5V → VCC。
2. 运行 **STC-ISP**（免费，Windows，官网下载
   <https://www.stcmicro.com/rjxz.html>）。macOS/Linux 可用开源的
   [`stcgal`](https://github.com/grigorig/stcgal) 或
   [`stc8prog`](https://github.com/IOsetting/stc8prog)。
3. 芯片选 **STC8G1K08A-8PIN**，选串口和 hex 文件。
4. **"输入用户程序运行时的IRC频率"必须填 35.000 MHz**（WS2812 时序全靠它）。
   其余选项照截图：`docs/images/stc-isp-settings.png`。
5. 点"下载/编程"，**再给目标板重新上电**（STC 引导程序只在冷启动时运行）。

> 注意：截图设置勾了"下次下载用户程序时擦除用户EEPROM区"，烧录会把记忆的
> 颜色/亮度重置为默认值；想保留就取消该勾选。
